...
def test_chandelier(config_default, test_source, test_symbol, time_begin, time_end, period, multiplier):
    ...
    assert compare_with_nan(chandelier.exit_short, ref_values_short.chandelier_exit)
    assert compare_with_nan(chandelier.exit_long, ref_values_long.chandelier_exit)
