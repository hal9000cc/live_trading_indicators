...
def test_mma(config_default, test_source, test_symbol, time_begin, time_end, period, a_timeframe):

    indicators = lti.Indicators(test_source, time_begin, time_end)
    ohlcv = indicators.OHLCV(test_symbol, a_timeframe)
    ema = indicators.MA(test_symbol, a_timeframe, period=period, ma_type='mma0')
