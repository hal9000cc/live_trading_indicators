...
При создании Indicators можно указать *with_incomplete_bar=True*, тогда будут получены данные последнего, не завершенного бара. См. пример выше.
### Коды торговых инструментов Binance
- Для спотового рынка полностью совпадают с кодом на binance (*btcusdt*, *ethusdt*, и т.д.)
- Для фьючерсного рынка **USD-M** имеют префикс **um/** (*um/btcusdt*, *um/ethusdt* и т.д.)
- Для фьючерсного рынка **COIN-M** имеют префикс **сm/** (*cm/btcusd_perp*, *cm/ethusd_perp* и т.д.)
